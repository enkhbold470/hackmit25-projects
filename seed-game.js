...
async function main() {
  try {
      ...
      allUsers.map((u, index) =>
        prisma.teamMember.create({
          data: {
            userId: u.id,
            teamId: team.id,
            status: index === 0 ? 'neutral' : index === 1 ? 'weakened' : index === 2 ? 'powered' : 'neutral',
          },
        })
      )
    );
    ...
    const sampleTransactions = [
      ...
      { restaurant: 'Burger Junction', amount: 18.50, days: 8 },
      { restaurant: 'Sushi Express', amount: 32.75, days: 10 },
      { restaurant: 'Taco Time', amount: 15.25, days: 12 },
      { restaurant: 'Chinese Garden', amount: 28.90, days: 15 },
    ];
